...
-- dependency_graph_minutes_dependency_calls_mv_v2

CREATE MATERIALIZED VIEW IF NOT EXISTS signoz_traces.dependency_graph_minutes_dependency_calls_mv_v2 TO signoz_traces.dependency_graph_minutes_v2
(
    `src` LowCardinality(String),
    `dest` String,
    `duration_quantiles_state` AggregateFunction(quantiles(0.5, 0.75, 0.9, 0.95, 0.99), Float64),
